...
names <- c('NC', 'FWO', 'Vf')
custom_names <- c("Case", "Free", "Verb-\nfinal")
named_vector <- setNames(custom_names, names)

#plot_model_set(m, manual_layout = positions)
model_sets <- plot_model_set(
    m, manual_layout = positions, edge_width = 3, curvature=0,
    text_size = 18, labels=named_vector,
    box_x=55, box_y=65,
    arrow = grid::arrow(type = 'closed', 30, grid::unit(12, 'points')))
#model_sets

ggsave(file="output/phylopath_model_sets_custom.svg", plot=model_sets, width=35, height=25)
ggsave(file="output/phylopath_model_sets_custom.tiff", plot=model_sets, width = 35, height = 25)
