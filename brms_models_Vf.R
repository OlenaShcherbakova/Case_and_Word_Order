...
model1 <- brm(
  ...
  data2 = list(A = A),
  family = "bernoulli",
  formula = Verb_final ~ 1 + (1 | gr(Glottocode, cov = A)),
...
save(model1, file = "output_models/model1_Vf.RData")
...
save(ranef_1, file = "output_models/random_effects_1_Vf.RData")

fixef_1 <- fixef(model1)
save(fixef_1, file = "output_models/fixed_effects_1_Vf.RData")
...
save(model2, file = "output_models/model2_Vf.RData")
...
save(ranef_2, file = "output_models/random_effects_2_Vf.RData")

fixef_2 <- fixef(model2)
save(fixef_2, file = "output_models/fixed_effects_2_Vf.RData")
...
save(model3, file = "output_models/model3_Vf.RData")

fitted <-
  fitted(model3) %>% as.data.frame() %>% bind_cols(grambank_phylopath_compl)
filepath_fitted <-
  paste0("output_tables/", "fitted_set1_model", "3_Vf", ".csv", collapse = "")
write.csv(fitted, filepath_fitted)
...
save(ranef_3, file = "output_models/random_effects_3_Vf.RData")

fixef_3 <- fixef(model3)
save(fixef_3, file = "output_models/fixed_effects_3_Vf.RData")
