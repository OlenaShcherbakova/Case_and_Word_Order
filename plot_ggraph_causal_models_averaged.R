...
custom_layout$name <- c("Case", "Flexible", "Verb-final")

#best model
load('output_models/phylopath_CI_nc_averaged.RData')

colnames(a_ci[["coef"]]) <- c("Verb-final", "Case", "Flexible")
rownames(a_ci[["coef"]]) <- c("Verb-final", "Case", "Flexible")


g <- igraph::graph_from_adjacency_matrix(a_ci$coef, weighted = TRUE)
l <-
  ggraph::create_layout(g, 'igraph', algorithm = "sugiyama") #algorithm does not work
...
p <- ggplot2::ggplot(l) +
  ggraph::geom_edge_arc(
    ...
    strength = 0.1,
    #completely straight lines strength = 0; > 0 - curved lines
    arrow = arrow,
    end_cap = ggraph::rectangle(box_x, box_y, 'mm'),
    start_cap = ggraph::rectangle(box_x, box_y, 'mm'),
    show.legend = FALSE,
    linejoin = c('bevel'),
    #lineend = c('round'), #"bevel" or "round" or "mitre"
    angle_calc = 'along',
    label_dodge = grid::unit(12, 'points'),
    label_size = 7
  ) +
  ggraph::geom_node_text(ggplot2::aes_(label = ~ name), size = 7) +
  ggraph::scale_edge_width_continuous(limits = c(0, max(igraph::E(g)$weight)),
                                      range = c(0, 2),
                                      guide = 'none') +
...
ggsave(
  file = "output/graph_phylopath_nc_averaged_model.jpg",
  plot = p,
  width = 10,
  height = 8,
  dpi = 300
...
ggsave(
  file = "output/graph_phylopath_nc_averaged_model.pdf",
  plot = p,
  width = 10,
  height = 8,
  dpi = 300
...
coef <- ggplot2::ggplot(df,
                        ggplot2::aes_( ~ path, ~ coef, ymin = ~ lower, ymax = ~
                                         upper)) +
  ...
  ggplot2::xlab('') +
  ggplot2::ylab('standardized regression\ncoefficient \U00B1 CI') + theme_classic() + theme(
...
ggsave(
  file = "output/graph_phylopath_nc_averaged_model_coefficients.jpg",
  plot = coef,
  width = 10,
  height = 8,
  dpi = 300
...
ggsave(
  file = "output/graph_phylopath_nc_averaged_model_coefficients.pdf",
  plot = coef,
  width = 10,
  height = 8,
  dpi = 300
...
joined <- p | coef
ggsave(
  file = "output/graph_phylopath_averaged.svg",
  plot = joined,
  width = 12,
  height = 9
)
...
ggsave(
  file = "output/graph_phylopath_averaged.jpg",
  plot = joined,
  width = 6,
  height = 8,
  dpi = 300
...
ggsave(
  file = "output/graph_phylopath_averaged.pdf",
  plot = joined,
  width = 6,
  height = 8,
  dpi = 300
