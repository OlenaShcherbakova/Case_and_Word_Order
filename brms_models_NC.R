...
model1 <- brm(
  ...
  data2 = list(A = A),
  family = "bernoulli",
  formula = Nominal_case ~ 1 + (1 | gr(Glottocode, cov = A)),
...
save(model1, file = "output_models/model1.RData")

fitted <-
  fitted(model1) %>% as.data.frame() %>% bind_cols(grambank_phylopath_compl)
filepath_fitted <-
  paste0("output_tables/", "fitted_set1_model", "1", ".csv", collapse = "")
...
save(ranef_1, file = "output_models/random_effects_1.RData")

fixef_1 <- fixef(model1)
save(fixef_1, file = "output_models/fixed_effects_1.RData")
...
save(model2, file = "output_models/model2.RData")

fitted <-
  fitted(model2) %>% as.data.frame() %>% bind_cols(grambank_phylopath_compl)
filepath_fitted <-
  paste0("output_tables/", "fitted_set1_model", "2", ".csv", collapse = "")
write.csv(fitted, filepath_fitted)
...
save(ranef_2, file = "output_models/random_effects_2.RData")

fixef_2 <- fixef(model2)
save(fixef_2, file = "output_models/fixed_effects_2.RData")
...
save(model3, file = "output_models/model3.RData")

fitted <-
  fitted(model3) %>% as.data.frame() %>% bind_cols(grambank_phylopath_compl)
filepath_fitted <-
  paste0("output_tables/", "fitted_set1_model", "3", ".csv", collapse = "")
write.csv(fitted, filepath_fitted)
...
save(ranef_3, file = "output_models/random_effects_3.RData")

fixef_3 <- fixef(model3)
save(fixef_3, file = "output_models/fixed_effects_3.RData")
