...
#brms/rstan works only for older R versions and rtools: https://github.com/stan-dev/rstan/issues/1011, so all brms analyses were run on Ubuntu 18.04.5 cluster 
...
# create output dir if it does not exist.		
if (!dir.exists(OUTPUTDIR_output_models)) { dir.create(OUTPUTDIR_output_models) }
...
load_data_NAs <- function(filename="data/GB_input.tsv") {
  ...
  grambank <- grambank[!with(grambank, is.na(GB070) | is.na(GB131) | is.na(GB132) | is.na(GB133) | is.na(GB136)),]
  grambank_phylopath_compl <- subset(x = grambank, select = c("Glottocode", "GB070", "GB131", "GB132", "GB133", "GB136"))
  ...
  grambank_phylopath_compl <- subset(x = grambank_phylopath_compl, select = c("Glottocode", "Verb_final", "Free_word_order", "Nominal_case"))
  
  # make sure we have factors here.
  for (col in colnames(grambank_phylopath_compl)) {
    grambank_phylopath_compl[[col]] <- as.factor(grambank_phylopath_compl[[col]])
  }
